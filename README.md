...
From the results we can see how the agents learn to move across the grid towards their goals. Particularly, the red agent learns where the green's goal is and tries to *beat* green by getting there first. So, since in each episode the starting position is random, the agent that wins is the closest one to the fixed goal and as can be expected, each agent wins about 50% of the times. 
